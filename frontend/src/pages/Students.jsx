...
function Students() {
  ...
  useEffect(() => {
    ...
    api.get("/students/").then((res) => { setStudents(res.data); setLoading(false) })
  }, [])
                ...
                {students.map((s, i) => (
                  <tr key={s.id} style={{ background: i % 2 === 0 ? "white" : "#f9f9f9", borderBottom: "1px solid #eee" }}>
                    ...
                    <td style={{ padding: "12px 16px" }}>
                      <span style={{ background: s.gender === "male" ? "#e3f2fd" : "#fce4ec", color: s.gender === "male" ? "#1565c0" : "#c62828", padding: "2px 10px", borderRadius: "12px", fontSize: "12px" }}>{s.gender}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
